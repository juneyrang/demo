		...
		table: {
			...
			formatter: {
				management: function(cell, formatterParams) {
					var menuSeq = cell.getRow().getData().MENU_SEQ;
					var html = 	'<div>' +
								'<i class="mdi mdi-menu m-auto text-primary" data-menu-no="' + menuSeq + '"></i>' + 
								'<i class="mdi mdi-airballoon m-auto text-primary" data-menu-no="' + menuSeq + '"></i>' + 
								'<i class="mdi mdi-trash-can-outline m-auto text-primary" data-menu-no="' + menuSeq + '"></i>' + 
								'</div>';
					return html;
				}
			},
