...
@Service("transShippingInvoiceService")
public class TransShippingInvoiceServiceImpl extends EgovAbstractServiceImpl implements TransShippingInvoiceService {

	private static final Logger LOGGER = LoggerFactory.getLogger(TransShippingInvoiceServiceImpl.class);
    ...
    public List getTransShippingInvoice(Map<String, Object> paramMap) {
		...
		return transShippingInvoiceDAO.getTransShippingInvoice(paramMap);   
    } 	

    @Override
    public List getTransShippingInvoiceDlgDetailRequest(Map<String, Object> paramMap) {
    	transDAO.updateTransSetLanguage(null);
		return transShippingInvoiceDAO.getTransShippingInvoiceDlgDetailRequest(paramMap);   
    } 
    
    @Override
    public List getTransShippingInvoiceDlgDetailPackingList(Map<String, Object> paramMap) {
    	transDAO.updateTransSetLanguage(null);
		return transShippingInvoiceDAO.getTransShippingInvoiceDlgDetailPackingList(paramMap);   
    } 
    ...
    public int saveTransShippingInvoiceDlgAttList(Map<String, Object> paramMap) throws Exception {
    	...
    	try {
			...
			cnt = transShippingInvoiceDAO.saveTransShippingInvoiceDlgAttList(paramMap);
    ...
    @Override
    public Map getTransShippingInvoiceMainProject(Map<String, Object> paramMap) throws Exception {
    	
    	int result_count = 0;
    	List<Map<String, Object>> subProjectList = new ArrayList<Map<String,Object>>();
    	Map<String, Object> mainProjectInfo = new HashMap<String, Object>();
    	Map<String, Object> resultMap = new HashMap<String, Object>();
    	
    	try {
    		
    		JSONArray selectList = JSONArray.fromObject(paramMap.get("list").toString());
        	for(int i = 0; i < selectList.size(); i++) {	    			
        		JSONObject obj = (JSONObject)selectList.get(i);
        		Map<String, Object> row = new ObjectMapper().readValue(obj.toString(), Map.class);
        		
        		List<Map<String, Object>> mainProjectList = transShippingInvoiceDAO.getTransShippingInvoiceMainProject(row);
        		result_count = Integer.parseInt(mainProjectList.get(0).get("RESULT_COUNT").toString());
        		
        		if(result_count > 0) {
        			row.put("CONDITION", "main");
        			
        			subProjectList = transShippingInvoiceDAO.getTransShippingInvoiceSubProject(row);
        			mainProjectInfo = ((List<Map<String, Object>>)transShippingInvoiceDAO.getTransShippingInvoiceMainProjectInfo(row)).get(0);
        			
        			resultMap.put("MAIN_PROJECT_ID", row.get("PROJECT_ID"));
					if(mainProjectInfo.get("FINAL_DESTINATION") != null)
						resultMap.put("FINAL_DESTINATION", mainProjectInfo.get("FINAL_DESTINATION").toString());
					if(mainProjectInfo.get("FINAL_DESTINATION_ADDR") != null)
						resultMap.put("FINAL_DESTINATION_ADDR", mainProjectInfo.get("FINAL_DESTINATION_ADDR").toString());
					resultMap.put("subProjectList", subProjectList);
					break; 
        		}
        	}
        	...
        	if(result_count == 0) {
            	...
            	for(int i = 0; i < selectList.size(); i++) {	    			
            		...
            		row.put("CONDITION", "internal");
            		List<Map<String, Object>> mainProjectInfoList = transShippingInvoiceDAO.getTransShippingInvoiceMainProjectInfo(row);
					
            		if(mainProjectInfoList.size() > 0) {
            			mainProjectInfo = mainProjectInfoList.get(0);
            			
						row.put("PROJECT_ID", mainProjectInfo.get("PROJECT_ID").toString());
						
						subProjectList = transShippingInvoiceDAO.getTransShippingInvoiceSubProject(row);
						
						resultMap.put("MAIN_PROJECT_ID", mainProjectInfo.get("PROJECT_ID").toString());
						if(mainProjectInfo.get("FINAL_DESTINATION") != null)
							resultMap.put("FINAL_DESTINATION", mainProjectInfo.get("FINAL_DESTINATION").toString());
						if(mainProjectInfo.get("FINAL_DESTINATION_ADDR") != null)
							resultMap.put("FINAL_DESTINATION_ADDR", mainProjectInfo.get("FINAL_DESTINATION_ADDR").toString());
			...
			if(result_count == 0) {
            	for(int i = 0; i < selectList.size(); i++) {	    			
            		...
            		List<Map<String, Object>> mainProjectInfoList = transShippingInvoiceDAO.getTransShippingInvoiceMainProjectInfo2(row);
        	...
        	for(int i = 0; i < selectList.size(); i++) {	    			
        		...
        		List<Map<String, Object>> checkList = transShippingInvoiceDAO.getTransShippingInvoiceStrategicCheck(row);
				
        		if(checkList.size() > 0 && checkList.get(0).get("CHECK_VALUE").toString().equals("N")) {
        			chkInvoice += "[" + checkList.get(0).get("INVOICE_NO").toString() + "]";
				}
			}
        	
        	resultMap.put("chkInvoice", chkInvoice);
    	}
    	catch (Exception e) {
    		System.out.println("!!!!!!" + e.toString());
    		throw new Exception(e.toString());
    ...
    public Map<String, Object> getTransShippingInvoiceProjectInfo(Map<String, Object> paramMap) throws Exception {
    	...
    	try {
	    	...
	    	resultMap.put("resultList", transShippingInvoiceDAO.getTransShippingInvoiceProjectInfo(paramMap));
	    	resultMap.put("chkInvoice", chkInvoice);
    ...
    public List getTransShippingInvoiceDlgTransRequestMailList(Map<String, Object> paramMap) {
    	transDAO.updateTransSetLanguage(null);
		return transShippingInvoiceDAO.getTransShippingInvoiceDlgTransRequestMailList(paramMap);   
    } 
    
    @Override
    public List getTransShippingInvoiceDlgTransRequestInvoiceList(Map<String, Object> paramMap) throws Exception {
    	...
    	try {
        	...
        	for(int i = 0; i < selectList.size(); i++) {	    			
        		...
        		Map<String, Object> paramRow = new HashMap<String, Object>();
        		paramRow.put("INVOICE_NO_ID", row.get("INVOICE_NO_ID"));
        		paramList.add(paramRow);
        	}  
        	
        	paramMap.put("InvoiceIds", paramList);
        	resultList = transShippingInvoiceDAO.getTransShippingInvoiceDlgTransRequestInvoiceList(paramMap);
    	}
    	catch (Exception e) {
    		System.out.println("!!!!!!" + e.toString());
    		throw new Exception(e.toString());
    ...
    public Map<String, Object> completeTransShippingInvoice(Map<String, Object> paramMap) throws Exception {
    	...
    	try {
			...
			if(!this.isVoyageDuplicate(paramMap)) {
				resultMap.put("result_status", "D");
				return resultMap;
			}
			
			/*
			noAttachInvList = this.isAttachShippingOrder(paramMap);
			if(!noAttachInvList.equals(""))
			{
				resultMap.put("result_status", "E");
				resultMap.put("result_data", noAttachInvList);
				return resultMap;
			}*/	
			
			if(paramMap.get("PROJECT_ID").toString().equals("J99999"))
			{
				Map<String, Object> voyageMap = new HashMap<String, Object>();
				voyageMap.put("USER_AD", paramMap.get("USER_AD"));
				voyageMap.put("SUBSTR_LENGTH", paramMap.get("USER_AD").toString().length() + 10);
				
				List<Map<String, Object>> JudanVoyageNoList = transShippingInvoiceDAO.getTransShippingInvoiceJudanVoyageNo(voyageMap); 
				String voyage_no =  JudanVoyageNoList.get(0).get("SEQ").toString();
						
				paramMap.put("VOYAGE_NO", voyage_no);
			}
	    	
			List<Map<String, Object>> orderSeqList = transShippingInvoiceDAO.getTransShippingInvoiceOrderSeq(paramMap); 
			paramMap.put("NEW_ORDER_MASTER_ID", orderSeqList.get(0).get("SEQ_ZSHP_SHIPPING_ORDER_M"));
			
			transShippingInvoiceDAO.saveTransShippingInvoiceNewOrder(paramMap);
			
			JSONArray invoiceList = JSONArray.fromObject(paramMap.get("invoiceList").toString());
	    	for(int i = 0; i < invoiceList.size(); i++) {	    			
	    		JSONObject obj = (JSONObject)invoiceList.get(i);
	    		Map<String, Object> row = new ObjectMapper().readValue(obj.toString(), Map.class);
	    		row.put("ORDER_MASTER_ID", paramMap.get("NEW_ORDER_MASTER_ID"));
	    		row.put("USER_AD", paramMap.get("USER_AD"));
	    		
	    		transShippingInvoiceDAO.saveTransShippingInvoiceOrder(row);	
	    	}	
	    	...
	    	for(int i = 0; i < updateMailList.size(); i++) {	    			
	    		...
	    		transShippingInvoiceDAO.saveTransShippingInvoiceOrderMail(row);	
	    	}
    		
	    	paramMap.put("ORDER_MASTER_ID", paramMap.get("NEW_ORDER_MASTER_ID"));
			if(setVoyageHistory(paramMap)) {
				System.out.println("[TransReqDetailService][createShippingOrder]::[INFO]::History저장성공.");
			}else {
				System.out.println("[TransReqDetailService][createShippingOrder]::[INFO]::History저장실패.");
			}	
	    		
			resultMap.put("result_data", paramMap.get("NEW_ORDER_MASTER_ID"));
	    	resultMap.put("result_status", "S");
					
    	}
    	catch (Exception e) {
			resultMap.put("result_msg", e.toString());
			resultMap.put("result_status", "F");
		}
        return resultMap;
    } 
    
	public boolean setVoyageHistory(Map<String, Object> paramMap) {
		boolean success = true;
		try {
	    	List<Map<String, Object>> historyInfoList = transShippingInvoiceDAO.getTransShippingInvoiceVoyageHistory(paramMap);  
	    	if(historyInfoList.size() > 0) {
	    		Map<String, Object> historyInfo = historyInfoList.get(0);
	    		
	    		String PARAM_ORDER_MASTER_ID = historyInfo.get("PARAM_ORDER_MASTER_ID") == null ? "" : historyInfo.get("PARAM_ORDER_MASTER_ID").toString();
	    		String PROJECT_ID = historyInfo.get("PROJECT_ID") == null ? "" : historyInfo.get("PROJECT_ID").toString();
	    		String PROJECT_DESC = historyInfo.get("PROJECT_DESC") == null ? "" : historyInfo.get("PROJECT_DESC").toString();
	    		String VOYAGE_NO = historyInfo.get("VOYAGE_NO") == null ? "" : historyInfo.get("VOYAGE_NO").toString();
	    		String TRANSPORT_TYPE = historyInfo.get("TRANSPORT_TYPE") == null ? "" : historyInfo.get("TRANSPORT_TYPE").toString();
	    		String TRANSPORT_TYPE_NAME = historyInfo.get("TRANSPORT_TYPE_NAME") == null ? "" : historyInfo.get("TRANSPORT_TYPE_NAME").toString();
	    		String LOADING_PORT = historyInfo.get("LOADING_PORT") == null ? "" : historyInfo.get("LOADING_PORT").toString();
	    		String DESTINATION_PORT = historyInfo.get("DESTINATION_PORT") == null ? "" : historyInfo.get("DESTINATION_PORT").toString();
	    		String M_CARGO_READY_DATE = historyInfo.get("M_CARGO_READY_DATE") == null ? "" : historyInfo.get("M_CARGO_READY_DATE").toString();
	    		String FINAL_DESTINATION_ADDR = historyInfo.get("FINAL_DESTINATION_ADDR") == null ? "" : historyInfo.get("FINAL_DESTINATION_ADDR").toString();
	    		String TRANSPORT_METHOD = historyInfo.get("TRANSPORT_METHOD") == null ? "" : historyInfo.get("TRANSPORT_METHOD").toString();
	    		String TRANSPORT_METHOD_NAME = historyInfo.get("TRANSPORT_METHOD_NAME") == null ? "" : historyInfo.get("TRANSPORT_METHOD_NAME").toString();
	    		String TRANSPORT_TERMS = historyInfo.get("TRANSPORT_TERMS") == null ? "" : historyInfo.get("TRANSPORT_TERMS").toString();
	    		String TRANSPORT_TERMS_NAME = historyInfo.get("TRANSPORT_TERMS_NAME") == null ? "" : historyInfo.get("TRANSPORT_TERMS_NAME").toString();
	    		String PACKAGE_COUNT = historyInfo.get("PACKAGE_COUNT") == null ? "" : historyInfo.get("PACKAGE_COUNT").toString();
	    		String TOTAL_PO_AMOUNT = historyInfo.get("TOTAL_PO_AMOUNT") == null ? "" : historyInfo.get("TOTAL_PO_AMOUNT").toString();
	    		String TOTAL_NET_WEIGHT = historyInfo.get("TOTAL_NET_WEIGHT") == null ? "" : historyInfo.get("TOTAL_NET_WEIGHT").toString();
	    		String TOTAL_GROSS_WEIGHT = historyInfo.get("TOTAL_GROSS_WEIGHT") == null ? "" : historyInfo.get("TOTAL_GROSS_WEIGHT").toString();
	    		String TOTAL_SHIPPING_VOLUME = historyInfo.get("TOTAL_SHIPPING_VOLUME") == null ? "" : historyInfo.get("TOTAL_SHIPPING_VOLUME").toString();
	    		String TOTAL_REVENUE_TON = historyInfo.get("TOTAL_REVENUE_TON") == null ? "" : historyInfo.get("TOTAL_REVENUE_TON").toString();
	    		String STATUS = historyInfo.get("STATUS") == null ? "" : historyInfo.get("STATUS").toString();
	    		
	    		historyInfo.put("PARAM_ORDER_MASTER_ID", PARAM_ORDER_MASTER_ID);
	    		historyInfo.put("PROJECT_ID", PROJECT_ID);
	    		historyInfo.put("PROJECT_DESC", PROJECT_DESC);
	    		historyInfo.put("VOYAGE_NO", VOYAGE_NO);
	    		historyInfo.put("TRANSPORT_TYPE", TRANSPORT_TYPE);
	    		historyInfo.put("TRANSPORT_TYPE_NAME", TRANSPORT_TYPE_NAME);
	    		historyInfo.put("LOADING_PORT", LOADING_PORT);
	    		historyInfo.put("DESTINATION_PORT", DESTINATION_PORT);
	    		historyInfo.put("M_CARGO_READY_DATE", M_CARGO_READY_DATE);
	    		historyInfo.put("FINAL_DESTINATION_ADDR", FINAL_DESTINATION_ADDR);
	    		historyInfo.put("TRANSPORT_METHOD", TRANSPORT_METHOD);
	    		historyInfo.put("TRANSPORT_METHOD_NAME", TRANSPORT_METHOD_NAME);
	    		historyInfo.put("TRANSPORT_TERMS", TRANSPORT_TERMS);
	    		historyInfo.put("TRANSPORT_TERMS_NAME", TRANSPORT_TERMS_NAME);
	    		historyInfo.put("PACKAGE_COUNT", PACKAGE_COUNT);
	    		historyInfo.put("TOTAL_PO_AMOUNT", TOTAL_PO_AMOUNT);
	    		historyInfo.put("TOTAL_NET_WEIGHT", TOTAL_NET_WEIGHT);
	    		historyInfo.put("TOTAL_GROSS_WEIGHT", TOTAL_GROSS_WEIGHT);
	    		historyInfo.put("TOTAL_SHIPPING_VOLUME", TOTAL_SHIPPING_VOLUME);
	    		historyInfo.put("TOTAL_REVENUE_TON", TOTAL_REVENUE_TON);
	    		historyInfo.put("STATUS", STATUS);	
	    		historyInfo.put("USER_AD", paramMap.get("USER_AD"));	    		
	    		
	    		transShippingInvoiceDAO.saveTransShippingInvoiceVoyageHistory(historyInfo);	
	    	}			
		} catch (Exception e) {
			success = false;
		} finally {
	...
	private String isAttachShippingOrder(Map<String, Object> paramMap) {
		String noAttachInvList = "";
		try {
	    	...
	    	for(int i = 0; i < invoiceList.size(); i++) {	    			
	    		...
	    		String documentGrpId = row.get("DOCUMENT_GRP_ID") == null ? "" : row.get("DOCUMENT_GRP_ID").toString();
	    		
	    		row.put("FILE_GRP_CD", documentGrpId);
	    		List<Map<String, Object>> attachFileList = transShippingRequestDAO.getTransShippingRequestDlgAttList(row);
	    		
	    		if(attachFileList.size() == 0) {
	    			noAttachInvList += "[" + row.get("INVOICE_NO").toString() + "]";
	    		}
	    	}	
		
			return noAttachInvList;
		}
		catch (Exception e) {
			return "Exception";
		}
    ...
    private boolean isVoyageDuplicate (Map<String, Object> paramMap) {
    	int isExist = 0;
    	boolean isUsable = true;
    	
    	String order_master_id = paramMap.get("ORDER_MASTER_ID").toString();
    	String project_id = paramMap.get("PROJECT_ID").toString();
    	String reqVoyageNo = "";
    	
    	try {
    		if(!project_id.equals("J99999"))
    		{
    			reqVoyageNo = paramMap.get("VOYAGE_NO").toString();
    			
    			if((order_master_id == null || "".equals(order_master_id) || "null".equals(order_master_id)) && reqVoyageNo != null)
    			{
    				List<Map<String, Object>> duplicateVoyageList = transShippingInvoiceDAO.getTransShippingInvoiceDuplicateVoyage(paramMap); 
    				isExist = Integer.parseInt(duplicateVoyageList.get(0).get("CNT").toString());
    ...
    public int sendMailTransShippingInvoice(Map<String, Object> paramMap) throws Exception {    	
    	...
    	try {
			...
			cnt = this.sendMailTransShippingInvoiceMailData(paramMap);
	...
	public int sendMailTransShippingInvoiceMailData(Map<String, Object> paramMap) {
		transDAO.updateTransSetLanguage(null);
		int cnt = 0;				
		...
		try {
			...
			cnt = this.sendMailTransShippingInvoiceMail(mailInfo);
	...
	public int sendMailTransShippingInvoiceMail(Map<String, Object> param) {
		int cnt = 0;
