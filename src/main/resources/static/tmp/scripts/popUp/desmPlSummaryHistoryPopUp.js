var dlgDesmPlSummaryHistoryGrid;

var v_desm_pl_summary_historycallback = null;
var v_desm_pl_summary_history_param;



function initDesmPlSummaryHistoryPopUp(param , callback) {
	v_desm_pl_summary_historycallback = callback;
    v_desm_pl_summary_history_param = param;

	$('#dlgDesmPlSummaryHistory').on('shown.bs.modal', function () {
		$('#dlgDesmPlSummaryHistory').click();
	});

	$('#dlgDesmPlSummaryHistory').on('hidden.bs.modal', function () {
	  	closeDesmPlSummaryHistoryPopUp();
	});

	$('#dlgDesmPlSummaryHistory').modal('show');


    initDesmPlSummaryHistoryPopUpCode();
}

function initDesmPlSummaryHistoryPopUpCode() {

	initDesmPlSummaryHistoryPopUpControls();
}


function initDesmPlSummaryHistoryPopUpControls() {


	TGSetEvent("OnCopy","dlgDesmPlDtlHistoryGrid",function(grid, txt){
	  copyToClipboard(txt);
	});

		var gridCode = getGridCode();
		var dlgDesmPlSummaryHistoryGridCol = {
		"Cfg": {
			"id"				: "dlgDesmPlSummaryHistoryGrid"
			, "CfgId"			: "dlgDesmPlSummaryHistoryGridCfg"
			, "SuppressCfg"		: "0"
			, "StyleLap"		: "0"
			, "Version"			: "2"
			, "CacheTimeout" : "100"
	...
	dlgDesmPlSummaryHistoryGrid = TreeGrid( {Layout:{Data : dlgDesmPlSummaryHistoryGridCol}, Data:{Data : {Body: [[]]}}, Text: gridLang},"dlgDesmPlSummaryHistoryGrid" );

	TGSetEvent("OnRenderFinish","dlgDesmPlSummaryHistoryGrid",function(grid){
		searchData();
	});
...
function closeDesmPlSummaryHistoryPopUp() {
	dlgDesmPlSummaryHistoryGrid.Dispose();
}
...
function searchData() {
	...
	$.ajax({
		...
		data: v_desm_pl_summary_history_param,
		success: function (data, textStatus, jqXHR) {

			dlgDesmPlSummaryHistoryGrid.Source.Data.Data.Body = [data.results];
			dlgDesmPlSummaryHistoryGrid.ReloadBody();
