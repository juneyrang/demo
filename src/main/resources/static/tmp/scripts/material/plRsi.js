...
function gridResize() {
	$("#gridRsi").width($(".table-responsive").width());
	$("#gridRsi").height($("body").height() - $("#tblSearchBox").height() - $("#toolbar").height() - 50);
}
...
function initControls() {
	...
	$('#btnRsiCreation').click(function () { btnRsiCreationClick(); return false; });
...
function initTable(){
	...
	var rsiGridCol = {"Cfg": {"id"				: "rsiGrid"
							...
							, "Cols" : [
							    {	"Name"	: "RSI_NO", "Width": "160", "Type": "Text" , "Class" : "gridLinkText gridCenterText", "CanMove" : "0", "CanEdit": "0", "Spanned":"1","OnClickCell"	: "openModal(Grid,Row,Col);" },
								...
								{	"Name"	: "HANDOVER_DATE", "Width": "130", "Type": "Text" , "Class" : "gridBorderText gridTextColor gridCenterText", "CanMove" : "0", "CanEdit": "0", "Spanned":"1" },
								{	"Name"	: "APPROVED_DATE", "Width": "130", "Type": "Text" , "Class" : "gridBorderText gridTextColor gridCenterText", "CanMove" : "0", "CanEdit": "0", "Spanned":"1" },
								{	"Name"	: "CLOSED_CODE", "Width": "130", "Type": "Text" , "Class" : "gridBorderText gridTextColor gridCenterText", "CanMove" : "0", "CanEdit": "0", "Spanned":"1" },
	...
	rsiGrid = TreeGrid( {Layout:{Data : rsiGridCol}, Data:{Data : {Body: [[]]}}, Text: gridLang},"gridRsi" );
	
	TGSetEvent("OnRenderFinish","rsiGrid",function(grid){
		gridReloadFn(grid);
		gridResize();
	});		
...
function btnRsiCreationClick() {
	
	var param = {"menuAuthList" : menuAuthList, "PROJECT_NO" : $('#txtProjectCode').val()};

	$('#dlgRsiPopUp').load("/desmRsiCreationPopUp.do",null,function(data, status, xhr) {
		if(status == "success"){
			
			initDesmRsiCreationPopUp(param, function (key, returnParam) {
...
function btnSearchClick() {
	...
	var pramData = {STATUS : $('#selStatus').val(), CLOSED_CODE : $('#selClosed').val(), RSI_NO : $('#txtRsiNo').val(), RSI_NAME : $('#txtRsiName').val(),
	...
	$.ajax({
		url: "/getDesmRsiList.do",		
		data: pramData,
		success: function (data, textStatus, jqXHR) {
			
			rsiGrid.Source.Data.Data.Body = [data.results];
			rsiGrid.Reload();
        }
    });
}
...
function openModal(grid,row,col) {
	var param = {"menuAuthList" : menuAuthList, "RSI_HEADER_ID" : row.RSI_HEADER_ID};
...
function openAttModal (grid,row,col) {
	...
	if(row.ATT_CNT != null && row.ATT_CNT != "" && row.ATT_CNT > 0){
		...
		$('#dlgRsiPopUp').load("/desmAttListPopUp.do",null,function(data, status, xhr) {
